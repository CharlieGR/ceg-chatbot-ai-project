...
# Without this, AgentCore attaches a managed long-term memory resource
# (SEMANTIC + SUMMARIZATION strategies) by default. That resource extracts
# facts across ALL sessions of the harness, not just within one — so a
# brand-new runtimeSessionId can "recall" a previous customer's ticket
# without ever calling the tool. The harness's own per-session state
# (what makes multi-turn field collection work) is unaffected by this;
# it only turns off cross-session recall.
...
def main():
    ...
    if existing:
        ...
        acc.update_harness(
            ...
            # update_harness wraps memory in an "optionalValue" (set vs.
            # clear) union, unlike create_harness's plain tagged union.
            memory={"optionalValue": MEMORY_CONFIG},
        )
    else:
