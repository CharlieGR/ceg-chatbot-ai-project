...
# Nova sometimes verbalizes its reasoning as literal <thinking>...</thinking>
# text in the same content the customer would see (see chat.py's
# ThinkingFilter for why we don't fight this via the prompt). Strip it here
# too so the judge model scores what a customer actually reads, not
# internal reasoning.
...
def invoke_harness_once(
    ...
    return {"final_output_text": strip_thinking(final_text)}
