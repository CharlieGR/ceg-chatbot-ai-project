...
class ThinkingFilter:
    """Strips <thinking>...</thinking> spans from a live text stream.

    Nova sometimes verbalizes its reasoning as literal <thinking> tags in
    the same text content the customer would see, despite being told not
    to. Suppressing that via the prompt turned out to hurt accuracy (the
    model got sloppier about copying real tool results, e.g. ticket IDs)
    so instead we let it reason freely and hide the tag content here,
    at display time, without touching what the model actually generates.
    """
    ...
    def feed(self, chunk):
        """Return the portion of `chunk` (plus any held-back buffer) that's
        safe to print now, holding back a partial tag at the boundary."""
        self.buffer += chunk
        out = []
        while True:
            tag = self.CLOSE if self.hiding else self.OPEN
        ...
        if not self.hiding:
            # Hold back a suffix that could be the start of "<thinking>".
            keep = 0
            for i in range(1, min(len(self.OPEN), len(self.buffer)) + 1):
                if self.buffer.endswith(self.OPEN[:i]):
                    keep = i
            if keep:
...
def invoke(rt, config, session_id, user_text, verbose=False):
    ...
    for event in event_stream(response):
        ...
        if "contentBlockStart" in event:
            ...
            if "text" in delta:
                visible = thinking_filter.feed(delta["text"])
                if visible:
